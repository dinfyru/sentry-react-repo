...
function stylusLoader(source) {
  ...
  const loaderOptions = (0, _util.getOptions)(this);
  ...
  const webpackConfig = this._compilation && (0, _util.isObject)(this._compilation.options) ? this._compilation.options : {};
  ...
  const styl = (0, _stylus.default)(source, options);
  ...
  if (resolveUrl) {
    ...
    } else if (!(0, _util.isObject)(resolveUrl)) {
      resolveUrl = {};
    }
    styl.define('url', _stylus.default.resolver(resolveUrl));
  }
  ...
  styl.render(async (err, css) => {
    if (err) {
      if (err.filename) {
        this.addDependency(_path.default.normalize(err.filename));
      }
      return callback(err);
    }
    const watchingDirs = {};
      ...
      for (const importData of options._imports) {
        ...
        if (options.watchDirs !== false) {
          ...
          if (!(dir in watchingDirs)) {
            this.addContextDependency(_path.default.normalize(dir));
